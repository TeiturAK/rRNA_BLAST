...
out=/crex/proj/uppstore2017145/V2/users/teitu/rRNA_tRNA/tRNA_seq/tRNAscan-SE
...
sbatch -A $proj -t 24:00:00 --mail-user=$mail -e $out/$fnam.err -o $out/$fnam.out \
-J $fnam -p node -N 1 runtRNAscan-SE.sh $fasta $out/results --log $out/log --gff $out/tRNA.gff --bed $out/tRNA.bed --fasta $out/tRNA.fa --missed $out/missed --hitsrc --progress --thread 20
